...
plot1 <- 
  ...
  annotate("segment",y=1.65,yend = 1.25,x=as.Date("2016-01-01"), xend=as.Date("2016-08-01"), linetype="dashed") +
...
judge_count %>% 
  ...
  annotate("segment", xend = 310, x = 200, yend = 0.9, y = 1.9,color="black", arrow=arrow(length = unit(0.03, "npc"))) + 
  my_theme +
...
#     TRUE ~ ""
#   )) %>% 
#   mutate(bond_type2 = case_when(
#     Bond_Type == "C-bond" ~ "Other",
#     Bond_Type == "IEM" ~ "Other",
#     Bond_Type == "No Bond" ~ "Other",
#     TRUE ~ Bond_Type
#    )) %>% 
#   mutate(Highest_Charge = ifelse(Highest_Charge == "",NA,Highest_Charge))
# 
# bond_court_obs_collapse <- bond_court_obs %>% 
#   group_by(after_reform,SARec_I, bond_type2, Highest_Charge) %>% 
#   summarise(freq = sum(ind)) %>% 
#   filter(!is.na(bond_type2)) %>% 
...
#        aes(y = freq, axis2 = after_reform, axis1 = Highest_Charge, axis3 = bond_type2)) +
...
#   )  +
#   my_theme +
#   scale_fill_manual(values = c("#095E7F", "#E5C528","#FF2E58","#99112C","#12BDFF"))
#   
# # bond_court_obs_collapse2 <- bond_court_obs_collapse %>% 
# #   filter(bond_type2 %in% c("D-bond","I-bond"))
# 
# plot7_2 <- ggplot(as.data.frame(bond_court_obs_collapse),
#        aes(y = freq, axis1 = after_reform, axis2 = bond_type2)) +
#   geom_flow(aes(fill = Highest_Charge, label = Highest_Charge), width = 1/12, alpha = 0.7)+
#   geom_stratum(width = 1/5, fill = "grey40", color="grey20") +
#   scale_x_discrete(limits = c("After Reform","Bond Type" )) +
#   geom_text(stat = "stratum", label.strata = TRUE,size = 3.5, color = "white", family = "Tahoma", face="bold") +
#   labs(
#     title="Significant Shift Away From Cash-Based D-Bonds and Towards I-Bonds \nAcross All Offense Types After the Reform Bill is Passed",
#     x = "",
#     y = "Frequency",
#     subtitle="Flow of Cases by Offense Type to Bond Decisions From August 2017 to October 2017",
#     fill = "Highest Charge",
...
#         legend.background = element_rect(fill="grey90", color = "#F8F8F8"))
#   # annotate("label", x = 1.4, y = 2150, family = "Tahoma", size=2, hjust=0, label = "Narcotics", color = "white", fill = "#095E7F" ) +
#   # annotate("label", x = 1.4, y = 1900, family = "Tahoma", size=2, hjust=0, label = "Person Crimes", color = "white", fill = "#B2AB21") +
#   # annotate("label", x = 1.4, y = 1750, family = "Tahoma", size=2, hjust=0, label = "Property Crimes", color= "white", fill = "#FF2E58") +
#   # annotate("label", x = 1.4, y = 1600, family = "Tahoma", size=2, hjust=0, label = "Unlawful Use of Weapon", color = "white", fill = "#99112C")
# 
# ggsave(here("data_output","alluvial_bond2.pdf"),plot7_2,device="pdf",width=8,height=5)
# 
# 
